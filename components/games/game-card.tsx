...
import { getOverallBestScore, readProgress } from '@/lib/games/progress';
import type { GameSummary } from '@/lib/games/types';

export function GameCard({ game, unlocked = false }: { game: GameSummary; unlocked?: boolean }) {
  const [bestScore, setBestScore] = useState(0);
  useEffect(() => {
    if (game.access === 'free' || unlocked) setBestScore(getOverallBestScore(readProgress(), game.slug));
  }, [game.access, game.slug, unlocked]);
