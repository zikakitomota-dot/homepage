...
export function FreebiePreview({ freebie, compact = false }: { freebie: Freebie; compact?: boolean }) {
  if (freebie.previewImage) {
    return (
      <div className={`relative bg-secondary ${compact ? 'aspect-[4/3]' : 'min-h-[360px] sm:min-h-[440px]'}`}>
        <Image
  ...
  return (
    <div
      className={`relative flex overflow-hidden bg-gradient-to-br ${
        playful ? 'from-amber-50 via-rose-50 to-sage-soft' : 'from-secondary/50 via-sage-soft to-blue-50'
    ...
    >
      ...
      <div className="absolute -bottom-8 -left-8 h-28 w-28 rounded-full bg-sage/30" aria-hidden="true" />
      <div className={`relative w-full rounded-2xl border border-white/80 bg-white/90 shadow-lg ${compact ? 'max-w-[220px] p-5' : 'max-w-sm p-8'}`}>
        <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-sage-soft text-sage-foreground">
          <Icon className="h-6 w-6" aria-hidden="true" />
        </span>
        <p className="mt-5 text-xs font-semibold uppercase tracking-[0.18em] text-sage-foreground">{freebieCategories[freebie.category].label}</p>
        ...
        <div className="mt-5 space-y-2" aria-hidden="true">
          ...
          <span className="block h-2 w-3/5 rounded-full bg-sage/40" />
        </div>
      </div>
    </div>
  );
