...
export function SalaryToHourlyCalculator() {
  ...
  const validation = useMemo(() => {
    const payLabel = mode === 'salary' ? 'Salary amount' : 'Hourly rate';
  ...
  const salaryPeriodLabel = salaryPeriod === 'annual' ? 'Annual salary' : salaryPeriod === 'monthly' ? 'Monthly salary' : 'Weekly salary';
  const quickSalaryExamples = salaryPeriod === 'annual'
    ? ['32000', '50000', '107000']
      ...
      <div id="salary-converter-panel" role="tabpanel" className="mt-6 grid gap-6 lg:grid-cols-[1fr_1.05fr]">
        <section aria-label="Pay and schedule inputs">
          <div className="grid gap-5 sm:grid-cols-2">
            {mode === 'salary' ? <>
              <NumberField id="salary-amount" label="Salary amount" value={salary} onChange={setSalary} step="0.01" placeholder="50000" />
              <div className="space-y-2"><Label htmlFor="salary-period">Salary period</Label><select id="salary-period" value={salaryPeriod} onChange={(event) => { setSalaryPeriod(event.target.value as SalaryPeriod); setCopyStatus('idle'); }} className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"><option value="annual">Annual salary</option><option value="monthly">Monthly salary</option><option value="weekly">Weekly salary</option></select><p className="text-xs leading-relaxed text-muted-foreground">Choose Monthly salary to convert monthly pay directly into an hourly rate.</p></div>
          ...
          <fieldset className="mt-7 rounded-xl border border-border/60 p-4"><legend className="px-2 font-semibold">Work schedule</legend><div className="grid gap-5 sm:grid-cols-3"><NumberField id="salary-hours-day" label="Hours per Day" value={hoursPerDay} onChange={setHoursPerDay} step="0.25" max="24" /><NumberField id="salary-days-week" label="Days per Week" value={daysPerWeek} onChange={setDaysPerWeek} step="0.5" max="7" /><NumberField id="salary-weeks-year" label="Weeks per Year" value={weeksPerYear} onChange={setWeeksPerYear} step="0.5" max="52" /></div><p className="mt-4 text-sm leading-relaxed text-muted-foreground">Reduce weeks per year if you take unpaid weeks away from work. Paid leave normally remains part of annual salary.</p><Button type="button" variant="outline" size="sm" className="mt-3 min-h-[44px]" onClick={useStandardSchedule}><WandSparkles className="mr-2 h-4 w-4" aria-hidden="true" />Use standard 8 × 5 × 52 schedule</Button></fieldset>

          {mode === 'salary' && <div className="mt-5"><p className="text-sm font-semibold">Quick {salaryPeriod} examples</p><div className="mt-2 flex flex-wrap gap-2">{quickSalaryExamples.map((value) => <Button key={value} type="button" variant="outline" size="sm" className="min-h-[44px]" onClick={() => { setSalary(value); setCopyStatus('idle'); }}>{formatOvertimeMoney(Number(value), currency)}</Button>)}</div></div>}
        ...
        <section className="rounded-2xl bg-blue-50/70 p-5" aria-labelledby="salary-result" aria-live="polite">
          <p className="text-sm font-semibold uppercase tracking-wider text-primary">Result</p><h2 id="salary-result" className="mt-2 text-xl font-bold">{mode === 'salary' ? 'Estimated Hourly Rate' : 'Estimated Annual Salary'}</h2>
          {result ? <><p className="mt-4 break-words text-3xl font-bold tracking-tight text-primary sm:text-4xl">{mode === 'salary' ? `${formatOvertimeMoney(result.hourly, currency)} per hour` : `${formatOvertimeMoney(result.annual, currency)} per year`}</p>{mode === 'salary' ? <div className="mt-4 rounded-xl border border-primary/15 bg-background/80 p-4"><p className="text-sm font-semibold">How this result was calculated</p><p className="mt-2 break-words text-sm leading-relaxed text-muted-foreground">{salaryPeriod === 'annual' ? <>{formatOvertimeMoney(Number(salary), currency)} annual salary</> : <>{formatOvertimeMoney(Number(salary), currency)} × {salaryPeriod === 'monthly' ? '12 months' : `${formatNumber(Number(weeksPerYear))} weeks`} = {formatOvertimeMoney(result.annual, currency)} annual salary</>}. Then {formatOvertimeMoney(result.annual, currency)} ÷ {formatNumber(result.hoursPerYear)} hours = <strong className="text-foreground">{formatOvertimeMoney(result.hourly, currency)} per hour</strong>.</p></div> : <p className="mt-2 text-sm text-muted-foreground">Hourly rate × annual working hours gives the estimated annual salary.</p>}<dl className="mt-5 grid gap-3 sm:grid-cols-2"><Result label={mode === 'salary' ? salaryPeriodLabel : 'Hourly rate'} value={mode === 'salary' ? formatOvertimeMoney(Number(salary), currency) : `${formatOvertimeMoney(Number(hourlyRate), currency)}/hour`} /><Result label="Hours per week" value={formatNumber(result.hoursPerWeek)} /><Result label="Weeks per year" value={formatNumber(Number(weeksPerYear))} /><Result label="Hours per year" value={formatNumber(result.hoursPerYear)} /></dl><h3 className="mt-6 font-semibold">Other pay equivalents</h3><dl className="mt-3 grid gap-3 sm:grid-cols-2"><Result label="Annual" value={`${formatOvertimeMoney(result.annual, currency)}/year`} /><Result label="Monthly" value={`${formatOvertimeMoney(result.monthly, currency)}/month`} /><Result label="Weekly" value={`${formatOvertimeMoney(result.weekly, currency)}/week`} /><Result label="Daily" value={`${formatOvertimeMoney(result.daily, currency)}/day`} /><Result label="Hourly" value={`${formatOvertimeMoney(result.hourly, currency)}/hour`} /></dl></> : <p className="mt-4 text-muted-foreground">Enter positive pay and schedule values to see an estimate. The calculator will not display an invalid result.</p>}
          <div className="mt-6 flex flex-col gap-3 sm:flex-row"><Button type="button" variant="outline" className="min-h-[44px]" onClick={reset}><RotateCcw className="mr-2 h-4 w-4" aria-hidden="true" />Reset</Button><Button type="button" className="min-h-[44px]" onClick={copyResult} disabled={!result}>{copyStatus === 'copied' ? <Check className="mr-2 h-4 w-4" aria-hidden="true" /> : <Clipboard className="mr-2 h-4 w-4" aria-hidden="true" />}{copyStatus === 'copied' ? 'Copied' : 'Copy Result'}</Button></div>
...
function NumberField({ id, label, value, step, max = String(maximumPay), placeholder, onChange }: { id: string; label: string; value: string; step: string; max?: string; placeholder?: string; onChange: (value: string) => void }) {
  return <div className="space-y-2"><Label htmlFor={id}>{label}</Label><Input id={id} type="number" inputMode="decimal" min="0" max={max} step={step} value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="h-11 text-base" /></div>;
}
