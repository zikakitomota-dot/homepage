...
const exampleRows: CategoryRow[] = [
  ...
  { id: 2, name: 'Quizzes', grade: '85', weight: '20' },
  { id: 3, name: 'Midterm', grade: '78', weight: '25' },
...
export function WeightedGradeCalculator() {
  ...
  return <Card className="overflow-hidden border-border/60 shadow-lg"><CardHeader className="border-b border-border/60 bg-card"><CardTitle className="flex items-center gap-2 text-2xl"><Scale className="h-6 w-6 text-primary" aria-hidden="true" />Calculate your weighted grade</CardTitle><p className="mt-2 text-sm leading-relaxed text-muted-foreground">Enter each assignment or category’s grade and its share of the course. For example, a 90% grade with a 20% weight contributes 18 percentage points.</p></CardHeader><CardContent className="p-4 sm:p-6"><div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
    <section aria-label="Weighted grade categories"><div className="hidden grid-cols-[minmax(0,1.5fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(6rem,0.9fr)_3rem] gap-3 px-3 pb-2 text-sm font-semibold text-muted-foreground md:grid" aria-hidden="true"><span>Assignment / Category</span><span>Grade (%)</span><span>Weight (%)</span><span>Weighted contribution</span><span>Remove</span></div><div className="space-y-3">{rows.map((row, index) => { const error = validateRow(row); const contribution = error ? null : weightedContribution(Number(row.grade || 0), Number(row.weight)); return <fieldset key={row.id} className="grid gap-3 rounded-xl border border-border/60 p-3 md:grid-cols-[minmax(0,1.5fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(6rem,0.9fr)_3rem] md:items-end"><legend className="sr-only">Weighted category row {index + 1}</legend><div className="space-y-2"><Label htmlFor={`weighted-name-${row.id}`} className="md:sr-only">Assignment / Category</Label><Input id={`weighted-name-${row.id}`} value={row.name} maxLength={80} onChange={(event) => updateRow(row.id, 'name', event.target.value)} className="h-11" /></div><NumberField id={`weighted-grade-${row.id}`} label="Grade (%)" value={row.grade} max={String(maximumGrade)} onChange={(value) => updateRow(row.id, 'grade', value)} /><NumberField id={`weighted-weight-${row.id}`} label="Weight (%)" value={row.weight} max="100" onChange={(value) => updateRow(row.id, 'weight', value)} /><div><span className="mb-2 block text-sm font-medium md:sr-only">Weighted contribution</span><output className="flex h-11 items-center rounded-md bg-secondary/50 px-3 font-semibold" title={contribution !== null ? `${row.grade || 0}% × ${row.weight}% = ${format(contribution)} percentage points` : undefined}>{contribution !== null ? `${format(contribution)} pts` : '—'}</output></div><Button type="button" variant="outline" size="icon" className="h-11 w-full md:w-11" onClick={() => removeRow(row.id)} disabled={rows.length === 1} aria-label={`Remove ${row.name || `category ${index + 1}`}`}><Trash2 className="h-4 w-4" aria-hidden="true" /></Button></fieldset>; })}</div><div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><Button type="button" onClick={addRow}><Plus className="mr-2 h-4 w-4" aria-hidden="true" />Add Assignment / Category</Button><Button type="button" variant="outline" onClick={loadExample}><Sparkles className="mr-2 h-4 w-4" aria-hidden="true" />Load Example</Button><Button type="button" variant="outline" onClick={reset}><RotateCcw className="mr-2 h-4 w-4" aria-hidden="true" />Reset</Button></div>{state.error && <p role="alert" className="mt-5 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm font-medium text-destructive">{state.error}</p>}</section>
    <WeightedResultPanel result={state.result} />
  </div></CardContent></Card>;
}

function WeightedResultPanel({ result }: { result: WeightedGradeResult | null }) {
  return <section className="rounded-2xl bg-blue-50/70 p-5" aria-labelledby="weighted-result" aria-live="polite"><p className="text-sm font-semibold uppercase tracking-wider text-primary">Result</p><h2 id="weighted-result" className="mt-2 text-xl font-bold">{result?.state === 'partial' ? 'Current Grade Across Entered Work' : 'Weighted Grade'}</h2>{result ? <>{result.state === 'overweight' ? <div className="mt-5"><p className="flex items-start gap-2 text-2xl font-bold text-amber-700"><TriangleAlert className="mt-1 h-6 w-6 shrink-0" aria-hidden="true" />Total weight exceeds 100%.</p><p className="mt-4 leading-relaxed text-muted-foreground">Total course weight cannot exceed 100%. Review the percentages entered before using the result.</p></div> : <><p className="mt-5 text-sm font-medium text-muted-foreground">{result.state === 'complete' ? 'Overall weighted grade' : 'Normalized grade across entered work'}</p><div className="mt-2 flex flex-wrap items-center gap-4"><p className="text-4xl font-bold tracking-tight text-primary">{format(result.currentGrade)}%</p><span className="flex min-h-14 min-w-14 items-center justify-center rounded-xl bg-primary px-3 text-2xl font-bold text-primary-foreground" aria-label={`Reference letter grade ${result.letter}`}>{result.letter}</span></div><p className="mt-4 leading-relaxed text-muted-foreground">{result.state === 'complete' ? `Your weighted contributions add up to an overall grade of ${format(result.currentGrade)}%.` : `This is your weighted average across the work entered so far. Those items represent ${formatWeight(result.totalWeight)}% of the full course.`}</p></>}
    <div className={`mt-6 rounded-xl border p-4 ${result.state === 'complete' ? 'border-emerald-300 bg-emerald-50' : result.state === 'overweight' ? 'border-amber-300 bg-amber-50' : 'border-border/60 bg-background'}`}><p className="flex items-start gap-2 font-semibold">{result.state === 'complete' && <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-600" aria-hidden="true" />}Total Weight: {formatWeight(result.totalWeight)}%{result.state === 'complete' ? ' — Complete' : ''}</p>{result.state === 'partial' && <p className="mt-2 text-sm leading-relaxed text-muted-foreground">You have entered {formatWeight(result.totalWeight)}% of the course weight. Add the remaining {formatWeight(100 - result.totalWeight)}% for a complete course grade.</p>}</div>
    ...
    {result.state !== 'overweight' && <p className="mt-4 text-sm leading-relaxed text-muted-foreground">Reference Letter Grade: <strong className="text-foreground">{result.letter}</strong>. Grading scales vary between schools, universities and countries. This uses a common US scale only as a general reference.</p>}
