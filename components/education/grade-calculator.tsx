...
export function GradeCalculator() {
  ...
  return <Card className="overflow-hidden border-border/60 shadow-lg"><CardHeader className="border-b border-border/60 bg-card"><CardTitle className="flex items-center gap-2 text-2xl"><GraduationCap className="h-6 w-6 text-primary" aria-hidden="true" />Calculate your grade</CardTitle><p className="mt-2 text-sm text-muted-foreground">Scores stay in your browser and are not saved or submitted.</p></CardHeader><CardContent className="p-4 sm:p-6">
    ...
    <div id="grade-panel" role="tabpanel" className="mt-6 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
      <section aria-label="Grade inputs">
        {mode === 'single' ? <div className="grid gap-5 sm:grid-cols-2"><NumberField id="points-earned" label="Points Earned" value={earned} onChange={setEarned} /><NumberField id="total-points" label="Total Points" value={possible} onChange={setPossible} /></div> : <>
          <div className="hidden grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(5rem,0.8fr)_3rem] gap-3 px-3 pb-2 text-sm font-semibold text-muted-foreground md:grid" aria-hidden="true"><span>Assignment</span><span>Score Earned</span><span>Total Points</span><span>Percentage</span><span>Remove</span></div>
          <div className="space-y-3">{rows.map((row, index) => { const rowError = validatePoints(row.earned, row.possible); const rowResult = rowError ? null : calculateGrade(Number(row.earned), Number(row.possible)); return <fieldset key={row.id} className="grid gap-3 rounded-xl border border-border/60 p-3 md:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(5rem,0.8fr)_3rem] md:items-end"><legend className="sr-only">Assignment row {index + 1}</legend><div className="space-y-2"><Label htmlFor={`assignment-name-${row.id}`} className="md:sr-only">Assignment</Label><Input id={`assignment-name-${row.id}`} value={row.name} maxLength={80} onChange={(event) => updateRow(row.id, 'name', event.target.value)} className="h-11" /></div><NumberField id={`assignment-earned-${row.id}`} label="Score Earned" mobileLabel value={row.earned} onChange={(value) => updateRow(row.id, 'earned', value)} /><NumberField id={`assignment-possible-${row.id}`} label="Total Points" mobileLabel value={row.possible} onChange={(value) => updateRow(row.id, 'possible', value)} /><div><span className="mb-2 block text-sm font-medium md:sr-only">Percentage</span><output className={`flex h-11 items-center rounded-md bg-secondary/50 px-3 font-semibold ${rowResult?.isExtraCredit ? 'text-emerald-700' : ''}`}>{rowResult ? `${formatPercentage(rowResult.percentage)}%` : '—'}</output></div><Button type="button" variant="outline" size="icon" className="h-11 w-full md:w-11" onClick={() => removeRow(row.id)} disabled={rows.length === 1} aria-label={`Remove ${row.name || `assignment ${index + 1}`}`}><Trash2 className="h-4 w-4" aria-hidden="true" /></Button></fieldset>; })}</div>
...
function validatePoints(earnedText: string, possibleText: string) {
  if (earnedText.trim() === '' || possibleText.trim() === '') return 'Enter both points earned and total points.';
  ...
  if (possible === 0) return 'Total points must be greater than zero.';
