  ...
  return <div className="min-h-screen bg-background"><SiteHeader /><main id="home">
    <section className="relative overflow-hidden border-b border-border/60">
      ...
      <div className="mx-auto grid max-w-[1200px] items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-24">
        <div className="flex flex-col items-start gap-6"><Badge variant="secondary" className="border-primary/20 bg-primary/10 text-primary">Practical resources for decisions and learning</Badge><h1 className="text-balance text-4xl font-bold leading-tight tracking-tight text-foreground sm:text-5xl lg:text-6xl">Understand the Task, Then Use the Tool</h1><p className="max-w-xl text-lg leading-relaxed text-muted-foreground">Zalea Studio publishes explained calculators for everyday decisions, learning activities that connect answers to understanding, and guides or printables that carry the task beyond the screen. The public resources are free; optional paid products stay in the shop.</p><div className="flex flex-col gap-3 sm:flex-row"><Button asChild size="lg" className="h-12 px-6"><Link href="#free-tools" data-ga-event="cta_click" data-ga-label="home_explore_free_resources">Explore Free Resources<ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" /></Link></Button><Button asChild size="lg" variant="outline" className="h-12 px-6"><Link href="/guides" data-ga-event="cta_click" data-ga-label="home_browse_guides">Read Practical Guides</Link></Button></div></div>
    ...
    <section id="free-tools" className="scroll-mt-20 border-b border-border/60 bg-secondary/30" aria-labelledby="resource-areas"><div className="mx-auto max-w-[1200px] px-4 py-16 sm:px-6 lg:px-8 lg:py-24"><div className="mx-auto mb-10 max-w-3xl text-center"><p className="text-sm font-semibold uppercase tracking-wider text-primary">Three ways to use Zalea</p><h2 id="resource-areas" className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Calculate, learn, or work through a practical task</h2><p className="mt-4 leading-relaxed text-muted-foreground">Finance, work, education and math tools handle everyday calculations. Learning games support focused practice. Guides and printables help plan, teach or record what happens next.</p></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{resourceAreas.map((area) => <ResourceCard key={area.href} {...area} />)}</div></div></section>
