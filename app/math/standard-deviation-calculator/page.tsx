...
const faqs = [
  ...
  { question: 'Is standard deviation the same as standard error?', answer: 'No. Standard deviation describes how spread out individual values are around their mean. Standard error describes uncertainty in an estimated mean and is a different calculation.' },
...
const guides = [
  ...
  { title: 'Sample vs Population Standard Deviation', body: <><p><strong>Sample standard deviation:</strong> use this when the entered values are a sample from a larger group. Its variance divides by n − 1.</p><p className="mt-3"><strong>Population standard deviation:</strong> use this when the values represent the entire population. Its variance divides by N.</p><p className="mt-3">The sample result is usually slightly larger because its denominator is smaller.</p></> },
  ...
  { title: 'Worked Example', body: <><p>For the dataset <strong>2, 4, 4, 4, 5, 5, 7, 9</strong>, the mean is <strong>5</strong> and the sum of squared deviations is <strong>32</strong>.</p><p className="mt-3">Population: √(32 ÷ 8) = <strong>2</strong><br />Sample: √(32 ÷ 7) ≈ <strong>2.13809</strong></p></> },
  { title: 'Mean, Standard Deviation and Standard Error', body: <><p>The mean describes the center of a dataset, while standard deviation describes how spread out its values are. Standard error instead describes uncertainty in an estimated mean; this calculator does not calculate standard error.</p><p className="mt-3">If you mainly need mean, median, mode and range, use the <Link href="/math/average-calculator" className="font-semibold text-primary hover:underline">Average Calculator</Link>.</p></> },
];
